...
export class UI {
    ...
    private liveImage: HTMLImageElement;
    private candyImage: HTMLImageElement;
    private candyWidth: number;
    private candyHeight: number;
    private candyFrame: number;
    private color: string;
    private ultimateBar: HTMLImageElement;
    ...
    private baseParticleGapX: number;
    private currentParticleGapX: number;
    private barAngle: number;
    private ultimateBarShaking: boolean;
    private ultimateCountNumbers: HTMLImageElement;
    private numbersWidth: number;
    private numbersHeight: number;
    constructor(game) {
        this.game = game
        this.fontSize = 50
        ...
        this.liveImage.src = 'secret/effects/heart.png'
        this.candyImage = new Image()
        this.candyImage.src = 'secret/effects/mini_candy.png'
        this.candyWidth = 40.5
        this.candyHeight = 100
        this.candyFrame = 0
        this.ultimateBar = new Image()
        this.ultimateBar.src = 'secret/effects/ultimate-progress3.png'
        this.test = new Image()
        this.test.src = 'secret/effects/ultimate-progress-full.png'
        this.ultimateCountNumbers = new Image()
        this.ultimateCountNumbers.src = 'secret/effects/numbers_ui.png'
        ...
        if(this.ultimateBarShaking && this.game.ultimateTimer === 0) {
            ...
            if(this.game.ultimateTimer > 0) {
                ...
                context.drawImage(this.ultimateCountNumbers, this.numbersWidth * frameX, 0,
                    this.numbersWidth, this.numbersHeight, this.barX + this.barWidth * 0.43, this.marginTop, this.numbersWidth/2, this.numbersHeight/2)
            }
        }
    ...
    chooseBarImg(enemyKilled) {
        ...
        switch (enemyKilled) {
            case 0: this.barFrameX = 0, this.barFrameY = 0,
                this.ultimateBar.src = 'secret/effects/ultimate-progress3.png', this.currentParticleGapX = 0
                break
            ...
            default: this.barFrameX = 0, this.barFrameY = 0,
                this.ultimateBar.src = 'secret/effects/ultimate-progress-full.png', this.currentParticleGapX = 9*basePixels
                break
        }
    }

    /*displauStatusText(context,gameOver) {
        if(gameOver) {
            context.textAlign = 'center'
            context.fillStyle = 'black'
            context.fillText('GAME OVER, press Enter to start again!', canvas.width/2, 200)
            context.fillStyle = 'white'
            context.fillText('GAME OVER, press Enter to start again!', canvas.width/2 + 2, 202)
        }
    }*/
}