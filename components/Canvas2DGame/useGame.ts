...
import {FlyingEnemy, CandyPointEnemy, GolemEnemy, YetiEnemy} from "./enemies";
import { UI } from './UI';
...
const useGame = () => {
    ...
    class Game {
        ...
        private readonly enemyInterval: number;
        ...
        constructor(width, height, restartGame) {
            this.width = width
            this.height = height
            this.speed = 0
            this.maxSpeed = 10
            ...
            this.input = new InputHandler(this, restartGame)
            this.UI = new UI(this)
            this.enemies = []
            this.particles = []
            this.collisions = []
        ...
        addEnemy() {
            ...
            if(this.speed > 0) {
                if (appearanceFrequency('30%')) this.enemies.push(new CandyPointEnemy(this))
                    ...
                    if(appearanceFrequency('80%')) this.enemies.push(appearanceFrequency('50%')
                        ? new GolemEnemy(this)
                        : new YetiEnemy(this))
                    else {
    ...
    useEffect(() => {
        ...
        const game = new Game(canvas.width, canvas.height, restartGame)
